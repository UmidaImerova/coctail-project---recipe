...
const category = document.querySelector('.categoryBtn');

category.addEventListener('click', (event) => {


  let categoryData = event.target.dataset['f'];

  const searchCategory = (categoryData) => {
    const categoryArr = [];
    for (let el of data) {
      let nameOfCategory = el.strCategory.toLowerCase();
      if (nameOfCategory.includes(categoryData.toLowerCase())) {
        categoryArr.push(el);
      }
    }
    console.log (categoryArr);
    clearContent();
    showRecipe(categoryArr)
  };
